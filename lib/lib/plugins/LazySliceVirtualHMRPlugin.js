...
class LazySliceVirtualHMRPlugin {
  constructor({
    ...
    this.virtualModulePath = path.resolve(process.cwd(), 'src', '__virtual__', 'hmr-cache-lazy.js');
    this.virtualRequest = '@hmr-cache-lazy';
    this.vmp = new VirtualModulesPlugin();
  }
  apply(compiler) {
    this.vmp.apply(compiler);
    compiler.hooks.beforeCompile.tapAsync('LazySliceVirtualHMRPlugin', (_params, callback) => {
      const absSliceDir = path.resolve(process.cwd(), this.sliceDir);
      const pattern = path.join(absSliceDir, '**/*.{js,jsx,ts,tsx}');
      const files = glob.sync(pattern, {
        absolute: true
      });
      const sliceFiles = files.filter(f => fs.readFileSync(f, 'utf8').includes('createLoadableSlice('));
