...
const Star = function Star(w, h, size, orbitA, orbitB, teta, padding) {
  ...
  // set own radius
  const radius = (0, _random.default)(1, 3);
  // set own alpha
  const alpha = (0, _random.default)(0.5, 0.9);
  // set speed
  const speed = (0, _random.default)(0.5, 0.8) * -1;
  ...
  const padX = (Math.random() > 0.5 ? 1 : -1) * (0, _random.default)(0, padding);
  const padY = (Math.random() > 0.5 ? 1 : -1) * (0, _random.default)(0, padding);
  x += padX;
  y += padY;
  this.teta = teta;
  this.x = x;
